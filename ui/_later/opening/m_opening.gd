...
@export_file("*.tscn") var next_scene: String # m m w anim
...
func _load_next_scene() -> void:
	...
	if show_loading_screen or status != ResourceLoader.THREAD_LOAD_LOADED:
		M_SceneLoader.change_scene_to_loading_screen()
	else:
		M_SceneLoader.change_scene_to_resource()
...
func _ready() -> void:
	M_SceneLoader.load_scene(next_scene, true)
