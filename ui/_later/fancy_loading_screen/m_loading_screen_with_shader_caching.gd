...
func can_load_shader_cache() -> bool:
	...
	M_SceneLoader.is_loading_scene(_cache_shaders_scene)
...
func _set_scene_loading_complete() -> void:
	...
	if can_load_shader_cache() and _caching_progress < 1.0:
		return
	M_SceneLoader._background_loading = false
	M_SceneLoader.set_process(true)
...
func _ready() -> void:
	M_SceneLoader._background_loading = true
