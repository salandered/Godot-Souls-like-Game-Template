...
func _update_text_from_file() -> void:
	...
	file_text = regex_replace_formatting(file_text) # after the regex_replace_imgs
...
func regex_replace_formatting(credits: String) -> String:
	var regex = RegEx.new()
...
func regex_replace_blockquotes(credits: String) -> String:
	var regex = RegEx.new()
	# (?m) enables multiline mode so ^ matches the start of any line, not just the string
	regex.compile("(?m)^> (.*)")
	
	# We wrap it in [left] to override the global center
	# We wrap in [indent] to add padding (makes the line length feel shorter)
	# Optional: Add [color] to make it look like a distinct quote/license block
	return regex.sub(credits, "[indent][left][color=#dddddd]$1[/color][/left][/indent]", true)
