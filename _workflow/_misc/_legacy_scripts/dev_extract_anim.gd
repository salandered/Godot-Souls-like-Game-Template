...
## DANGER TODO:
# Script is no longer in need and is not supported. Keeping it because it has universal logic implemented
# Need to parse this into task agnostic utilities
...
func process_glb_file() -> bool:
	...
	for anim_name in animation_list:
		...
		# TODO: deprected. Add param tracks to the animation itself
		if PARAM:
			var unified_param_name := unified_name + "-param" # A.to_backend_anim(unified_name)
...
func _create_param_animation(base_animation: Animation, unified_param_name: String) -> Animation:
	...
	var states_db_path_str := "StatedDB" # StatesDatabase.STATES_DB
