...
func initialize() -> void:
	...
	[
		...
		[GlobalUIInfo.SIG_dtc_b_overlay_panel_value_changed, _on_SIG_dtc_value_changed_section_op],
	]
)
...
func _on_SIG_dtc_value_changed_section_op(payload: Dictionary[StringName, Variant]):
	if not camera: return
	if not camera.is_node_ready(): return
