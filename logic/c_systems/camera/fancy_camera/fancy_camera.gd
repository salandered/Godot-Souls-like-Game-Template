...
# todo keep them in degs, and rads are in math
		...
		FREE_STATE_NAME:
			...
			if input_.target_lock.tap_or_double_tap():
				...
				if found_target:
					...
					__log_("", em.gray_x, "Can't lock (no target found)")
		LOCKED_STATE_NAME:
...
func _switch_free_from_locked(reason: String = ""):
	if __LOG_B(): __log_("LOCKED -> FREE", "reason", reason)
	locked_target = socket
	current_state = free_state
...
func _switch_locked_from_free(found_target: EnemyCameraTarget):
	if __LOG_B(): __log_("FREE -> LOCKED", __Cvec(), __free_boom(), "target=", found_target.pp_name())
