...
func is_ended() -> bool:
	...
	if not _all_will_do_is_done():
		if __ELA(): _reason += "some WillDo not done"
		_r = false
		...
		if __ELA(): _reason += "some finite substate not ended"
		_r = false
	# just small tolerance so we can't end on first frame or something
	elif time_spent() <= 1.0:
		if __ELA(): _reason += "works < 1.0s, give us some time"
		_r = false
	else:
	...
	match current_substate.state_name:
		PHES.Leaf.pursue:
			if pursue_for.is_done():
				if __ELA(): _reason += "pursue_for.is_done |"
				if dist > config.REAL_FAR():
					...
					if __ELA(): _reason += "dist > TOO_CLOSE"
				...
				if __ELA(): _reason += "pursue_for not done |"
				if dist < config.TOO_CLOSE():
					...
					if __ELA(): _reason += "pursue while we can"
	
		PHES.Leaf.orbit:
			if orbit_for.is_done():
				if __ELA(): _reason += "orbit_for.is_done |"
				if dist > config.CLOSE_TO_ORBIT() + 2:
					_next_state = PHES.Leaf.pursue
					if __ELA(): _reason += "dist > CLOSE_TO_ORBIT + 2"
				elif dist > config.ORBIT_RAD():
					if __ELA(): _reason += "dist > ORBIT_RAD"
		...
		PHES.Leaf.combat_idle:
			if idle_for.is_done():
				...
				# if with not_dodge_back_for, we help player
					...
					if __ELA(): _reason += "idling while we can"
	
		PHES.dodge_back_series, PHES.dodge_playful, PHES.Leaf.jump_towards:
			if current_substate.is_ended():
				if __ELA(): _reason += "dodge or jump_towards ended => _distance_to_pursue_sbs"
				var _verdict := _distance_to_pursue_sbs(_next_state, _reason)
				_next_state = _verdict.next_state
				if __ELA(): _reason += "|" + _verdict.get_reason()
			else:
				if __ELA(): _reason += "dodge or jump not ended"
