...
func _connect_signals(is_connect: bool):
	if is_connect:
		SigUtils.safe_connect(GlobalSignal.SIG_phe_state_changed, _on_SIG_phe_state_changed)
		SigUtils.safe_connect(GlobalSignal.SIG_phe_state_reset, _on_SIG_phe_state_reset)
		SigUtils.safe_connect(GlobalSignal.SIG_phe_weapon_hit_data_set, _on_SIG_phe_weapon_hit_data_set)
	else:
		SigUtils.safe_disconnect(GlobalSignal.SIG_phe_state_changed, _on_SIG_phe_state_changed)
		SigUtils.safe_disconnect(GlobalSignal.SIG_phe_weapon_hit_data_set, _on_SIG_phe_weapon_hit_data_set)
...
func _on_SIG_phe_state_changed(payload: Dictionary[String, Variant]):
	...
	selected_label.set_label_text(pp_string, dlc_all_features_on)
...
func _on_SIG_phe_state_reset(payload: Dictionary[String, Variant]):
	...
	selected_label.set_label_text("", dlc_all_features_on)
		

func _on_SIG_phe_weapon_hit_data_set(payload: Dictionary[String, Variant]):
	_on_SIG_hit_data_payload(attack_info_phe, payload, 16.0, 40.0, dlc_all_features_on)
