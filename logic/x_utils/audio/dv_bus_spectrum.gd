...
@export var audio_bus_name := Constants.SFX_ASP_BASE_BUS_ID
...
func _process_audio_data(delta: float):
	...
	for bar_idx in range(bars_count):
		var ih = bar_idx + 1
		var hz: float = MathUtil.lerp_log(MIN_HZ, MAX_HZ, ih / float(bars_count))
...
func _get_freq_at_index(i: int, offset: float = 1.0) -> float:
	...
	return MathUtil.lerp_log(MIN_HZ, MAX_HZ, t)
