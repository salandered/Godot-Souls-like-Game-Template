...
## queries the physics space, so it detects the "best" contact available
static func get_area_contact_point(my_area: Area3D, target_area: Area3D) -> Vector3:
	...
	if not result.is_empty():
		return result["point"] # returns contact position in Global space
		
	return Vector3.INF


## finds the first active collision shape
