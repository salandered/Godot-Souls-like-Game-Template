...
func check_transition(delta: float) -> String:
	...
	if works_longer_than(fatigue):
		print_.prefix("SE", "follow decision: fatigue => idle         " + str(get_progress()) + "      " + str(fatigue))
		return SEState.idle

	if distance_to_player() < me.attack_distance:
		if ra.chance(aggression * 0.5):
			print_.prefix("SE", "follow decision: <attack_distance => attack")
			return SEState.attack
		elif ra.chance(aggression):
			print_.prefix("SE", "follow decision: <attack_distance but ra => idle")
			return SEState.idle
		else:
			print_.prefix("SE", "follow decision: <attack_distance => attack")
			return SEState.attack
	...
	if distance_to_player() > me.sight_distance:
		print_.prefix("SE", "follow decision: >sight_distance => backtrack")
