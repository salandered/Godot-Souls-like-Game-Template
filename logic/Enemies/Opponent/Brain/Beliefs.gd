...
func have_stamina_for_state() -> bool:
	return character_resources.stamina > actions_container.get_action("shield_throw").stamina_cost

func have_state_charge() -> bool:
	return actions_container.get_action("shield_throw").charges > 0

func state_speed() -> float:
	# TODO tie with spell
	return 20
...
func in_players_attack_zone(point: Vector3) -> bool:
	# required we call it presuming the current state is an attack
	return point.distance_to(player.current_state_initial_position()) < player_attack_radius()
...
#func players_state_is_attackable() -> bool:
	#var future_player_pos = player.get_current_state_position_after(character_attack_windup_time())
...
func player_position_position_after(time: float) -> Vector3:
	var future_pos = player.get_current_state_position_after(time)
	#$"../debug_sphere".global_position = future_pos
	return future_pos

func player_attack_is_punishable() -> bool:
	var geometry_allows = player.current_state_posttracking_radius() + character_attack_radius() > distance_to_player()
...
func shield_throw_arm_length() -> float:
	# TODO tie with state throw action
	return 0.318

func shield_throw_projectile_emit_timing() -> float:
	# TODO tie with state throw action
	return 0.919

func state_restore_duration() -> float:
	# TODO tie with state throw action
	return 3.6

func player_is_reloading_state() -> bool:
	return player.get_current_state().state_name == "shield_throw_reload"
