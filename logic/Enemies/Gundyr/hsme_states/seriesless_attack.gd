...
@export var next_attacks: Array[BaseHSMEState]
...
func check_transition(_delta) -> VerdictHSM:
	if player_too_far():
		...
		return VerdictHSM.new(gapclosing_method)
	if player_too_close():
		return VerdictHSM.new("kick")
	if works_longer_than(get_animation_length()):
		return VerdictHSM.new(next_attacks.pick_random().state_name)
