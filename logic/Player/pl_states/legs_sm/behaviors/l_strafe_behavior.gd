...
var _non_strafe_timer: DelayTimer = DelayTimer.new()

func _ready():
	_non_moving_timer.initialise(TO_STOP_DELAY)
	_non_strafe_timer.initialise(TO_FORWARD_BACK_DELAY)
...
func _decide_move_action(input: InputPackage, delta: float) -> Dictionary:
	...
	if is_reverse_moving(input):
		...
		if input.reverse_data.is_reversed_strafe(): # A-D
			...
			next_action_name = supported_actions.by_name(Leg.Act.combat_walk)
	...
	if next_action_name == supported_actions.by_name(Leg.Act.strafe):
		_non_strafe_timer.reset()

	if next_action_name == supported_actions.by_name(Leg.Act.combat_walk):
		if not _non_strafe_timer.update(delta):
			reason += pp.s("\n\t\t\t", next_action_name, em.red_x, "UPD declined: timer. Would be current")
