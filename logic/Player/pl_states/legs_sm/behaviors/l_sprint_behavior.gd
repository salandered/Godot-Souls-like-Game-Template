	...
	match curr_motion_type:
		MotionType.IDLE:
			if is_moving(input_) and curr_action.works_longer_than(IDLE_COMMIT):
				...
				__log_decision_data(input_, pp.compare_w("works >", "commit", IDLE_COMMIT), next_action_name)
		...
		MotionType.START:
			if is_moving(input_):
				if curr_action.time_remaining_for_smooth_switch(supported_actions.default_by_motion(MotionType.START)) < 0.05:
					...
					__log_decision_data(input_, "time for smooth sw < 0.05", next_action_name)
			else:
		...
		MotionType.STOP:
			if is_moving(input_):
				if curr_action.works_longer_than(STOP_RESUME_COMMIT):
					...
					__log_decision_data(input_, pp.compare_w("works >", "commit", STOP_RESUME_COMMIT), next_action_name)
			else:
				if curr_action.time_remaining() < 0.5: # curr_action.works_longer_than(STOP_COMMIT) and
					next_action_name = supported_actions.default_by_motion(MotionType.IDLE)
					__log_decision_data(input_, pp.compare_w("time_remaining >", "0.1", STOP_COMMIT), next_action_name)
