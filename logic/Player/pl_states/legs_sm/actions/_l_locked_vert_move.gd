...
func _update_walk_direction(input: InputPackage, on_enter: bool = false) -> DualDirection.Dir:
	var new_dir = DualDirection.Dir.PRIMARY if input.forward_input > 0.0 else DualDirection.Dir.SECONDARY
...
func on_enter_action(input: InputPackage) -> void:
	curr_direction.set_direction(_update_walk_direction(input, true))
...
func _switch_animation():
	...
	if next_anim.anim_id == anim.anim_id:
		...
		return
	elif anim.anim_id in curr_direction.get_anims():
