...
# 		if current_action.acts_longer_than(releases_priority) and best_input != "jog": # TODO != base locomoti
# 			return best_input
...
# 	input.behaviour_names.append("jog") # TODO smth like append default locomotion mode that is walk/jog/crouc
# 	return input

# #func on_enter_behaviour(input : InputPackage):
# #	switch_action_to("charge", input)

# func choose_initial_behaviour(input : InputPackage):
# 	switch_action_to("charge", input)
# 	legs_behaviour = legs.behaviour_by_name("double_legs")
# 	#legs_behaviour = legs.behaviour_by_name("combat_walk_legs")
...
# 		legs.switch_to(legs.behaviour_by_name("combat_walk_legs"), input)
# 		switch_action_to("hold", input)
# 		return
# 	if current_action.action_name == "hold" and not input.behaviour_names.has("SNS_charged_attack"):
# 		switch_action_to("release", input)
# 		legs.switch_to(legs.behaviour_by_name("double_legs"), input)
# 		return

# func setup_legs_animator(previous_action : LegsAction, _input : InputPackage):
# 	print("requested legs animator setup")
# 	double_action = legs_behaviour.actions.get_by_name("double")
