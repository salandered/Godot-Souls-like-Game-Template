...
func switch_action_to(next_action: String, input: InputPackage):
	if current_action:
		print_.prefix("=PS=", "switching action from " + current_action.action_name + " to " + next_action)
		current_action.on_exit_action() # TODO: _on_exit_action ?
	current_action = container.action_by_name(next_action)
	current_action.animate()
...
func _on_enter_state(input: InputPackage):
	...
	## - And then we call `on_enter_state` method that is empty in the base and requires overriding in heirs.
