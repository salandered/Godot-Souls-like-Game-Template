extends Node
## Torso machine consists of states called torso_behaviors
class_name TorsoBehavior
...
func map_with_dictionary(input: InputPackage, map: Dictionary):
	for action in input.input_actions:
		if map.keys().has(action):
			input.behavior_names.append(map[action])
	return input
...
func try_queue_move(new_queued_move: String):
	...
	elif torso.behaviors[new_queued_move].priority > torso.behaviors[queued_move].priority:
		queued_move = new_queued_move
...
func try_force_move(new_forced_move: String):
	...
	elif torso.behaviors[new_forced_move].priority >= torso.behaviors[forced_move].priority:
		forced_move = new_forced_move


func _update(input: InputPackage, delta: float):
	legs.current_behavior.update(input, delta)
	update(input, delta)

func update(_input: InputPackage, _delta: float):
	pass
## - And then we call the real `on_enter_state` method that is empty in the base and requires overriding in heirs.
func _on_enter_behavior(input: InputPackage):
	...
	legs_behavior.torso_behavior = self
	legs.switch_to(legs_behavior, input)
	on_enter_behavior(input)
