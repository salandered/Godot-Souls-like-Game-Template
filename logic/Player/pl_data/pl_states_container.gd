...
var node_to_pl_action: Dictionary = { # { Node name : ActionData }
	"_DoubleAction": ActionData.new(PS.for_double, PS.Act.double, A.fake_anim, MotionType.IDLE),
	
	#
	"DodgeAction": ActionData.new(PS.dodge, PS.Act.dodge, A.dodge.dodge_R, MotionType.START),

	## air
	"JumpSprintAction": ActionData.new(PS.jump_sprint, PS.Act.jump_sprint, A.air.jump_sprint, MotionType.START),
	"MidairAction": ActionData.new(PS.midair, PS.Act.midair, A.air.midair, MotionType.LOOP),
	"LandingSprintAction": ActionData.new(PS.landing_sprint, PS.Act.landing_sprint, A.air.landing_sprint, MotionType.LOOP),

	## attacks
	"AxeSlice1Action": ActionData.new(PS.axe_slice_1, PS.Act.axe_slice_1, A.attack.axe_slice_1, MotionType.IDLE),
	"AxeSlice2Action": ActionData.new(PS.axe_slice_2, PS.Act.axe_slice_2, A.attack.axe_slice_2, MotionType.IDLE),
	"AttackFromRunAction": ActionData.new(PS.attack_from_run, PS.Act.attack_from_run, A.attack.attack_from_run, MotionType.STOP),
}

var pl_action_data_list: Array[ActionData] = [

	ActionData.new(PS.roll, PS.Act.roll, A.roll, MotionType.START),
	ActionData.new(PS.death, PS.Act.death, A.death, MotionType.IDLE),

	## attacks
	ActionData.new(PS.longsword_1, PS.Act.longsword_1, A.attack.axe_slice_1, MotionType.IDLE),
	ActionData.new(PS.longsword_2, PS.Act.longsword_2, A.attack.axe_slice_2, MotionType.IDLE),

	ActionData.new(PS.staggered, PS.Act.staggered, A.combat.staggered, MotionType.IDLE),
	ActionData.new(PS.parry, PS.Act.parry, A.combat.parry, MotionType.IDLE),
	ActionData.new(PS.parried, PS.Act.parried, A.combat.parried, MotionType.IDLE),
	ActionData.new(PS.riposte, PS.Act.riposte, A.combat.riposte_attack, MotionType.IDLE),
]
