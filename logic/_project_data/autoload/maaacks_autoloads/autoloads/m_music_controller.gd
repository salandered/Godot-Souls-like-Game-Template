...
func fade_out(duration: float = 0.0) -> Tween:
	...
	if music_stream_player:
		tween.tween_property(music_stream_player, PropC.VOLUME_DB, MINIMUM_VOLUME_DB, duration)
	return tween
...
func blend_to(target_volume_db: float, duration: float = 0.0) -> Tween:
	if not is_zero_approx(duration):
		var tween := create_tween()
		tween.tween_property(music_stream_player, PropC.VOLUME_DB, target_volume_db, duration)
		return tween
