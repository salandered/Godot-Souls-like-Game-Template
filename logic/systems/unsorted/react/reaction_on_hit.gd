...
static func _pick_bone_mask_for_enemy(hit: HitData, state_name: String) -> Array[int]:
	...
	_bone_mask = u.safe_get_dict_key(enemy_state_to_bone_mask, state_name, BoneMask.get_upper_body(), Fallback.SOFT)
...
static func _pick_bone_mask_for_player(hit: HitData, action_name: String) -> Array[int]:
	var _bone_mask: Array[int]
	_bone_mask = u.safe_get_dict_key(player_action_to_bone_mask, action_name, BoneMask.get_upper_body_with_hips(), Fallback.SOFT)
