...
func attack_in_series_passed_marker(curr_series_number_: int, curr_sbs: BasePHELeaf, target_sbs_name_: StringName, marker_name: StringName) -> bool:
	...
	if curr_sbs.passed_marker(marker_name):
		if __LOG_B(): __log_upd("captured attack_in_series_passed_marker", curr_series_number_, curr_sbs.state_name, target_sbs_name_, marker_name)
		return true
...
func _is_ended() -> bool:
	...
	if _current_substate == null:
		if __LOG_B(): __log_upd('_is_ended', "_current_substate is null", 'curr attack idx', current_attack_number, _chosen_attack_series)
		return true

	if dist_to_player_greater(PL_DIST_TO_END) and condition_to_end(_current_substate):
		if __LOG_B(): _reason += "dist > PL_DIST_TO_END and condition_to_end (hard stop if player is far way and we ended current attack) "
		_r = true
	elif not condition_to_end(_current_substate):
		if __LOG_B(): _reason += "not condition_to_end (in the middle of curr attack)"
		_r = false
	elif current_attack_number + 1 < _chosen_attack_series.size():
		if __LOG_B(): _reason += "have more attacks to do in the combo"
		_r = false
	else:
...
func check_substate_transition(delta: float, current_substate: BasePHEState, _next_state: StringName, _reason: String) -> VerdictPH:
	...
	if condition_to_next_switch(current_substate_casted):
		...
		if __LOG_B(): __log_upd("attack index + 1. curr/next", current_attack_number, _next_index)
		if _next_index < _chosen_attack_series.size(): # next attack exists
			...
			if __LOG_B(): _reason += pp.s("advancing attack series", "attacks remaining", _remaining)
			...
			if not _series_forgotten.is_initialized():
				...
				if __LOG_B(): _reason += pp.s("0.1 passed, one more move")
				_series_forgotten.turn_off()
...
func choose_initial_substate(_next_state: StringName, _reason: String) -> VerdictPH:
	...
	if _chosen_attack_series.is_empty():
		if __LOG_B(): _reason += "No valid combo found"
	else:
		current_attack_number = 0
		_next_state = _chosen_attack_series[current_attack_number]
		if __LOG_B(): _reason += "picked combo, starting with first attack"
	
	if __LOG_B(): __log_ent("Chosen combo:", _chosen_attack_series, "| Total attacks:", _chosen_attack_series.size())
	return VerdictPH.new(_next_state, _reason)
