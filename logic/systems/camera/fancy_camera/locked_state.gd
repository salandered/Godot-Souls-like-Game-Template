...
func _rotate_boom_locked(delta: float) -> void:
	...
	if desired_dir_xz.length_squared() < Const.EPSILON_5: return
	...
	if not blend_timer.is_complete():
		...
		var t_eased := MathUtils.ease_in_out(t_linear)
		...
		if current_hor_len < Const.EPSILON_5:
	...
	# __log_("[~~LOCK UPD rot ", FrameUtils.sfr(), "]",
