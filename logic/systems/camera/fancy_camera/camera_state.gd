...
func vertical_mouse_movement(d_x: float, d_y: float, boom: Vector3) -> Vector3:
	...
	var vertical_axis := boom.cross(Vector3.UP)
	var axis_len := vertical_axis.length()
	
	if axis_len < Const.EPSILON_5: # axis degenerate near straight-up/down — ignoring this frame
	...
	var theta_now := acos(clampf(boom.normalized().dot(Vector3.UP), -1.0, 1.0))

	# Predict result of the raw input
	var predicted := boom.rotated(vertical_axis, rot_angle)
	...
	if abs(theta_pred - theta_target) < Const.EPSILON_7:
		boom = predicted # no breach — take full step
	else:
		# breach: scale the rotation proportionally so land on the rail
		var denom := theta_pred - theta_now
		if abs(denom) > Const.EPSILON_9: # why so many different epsilons ...
			...
			boom = boom.rotated(vertical_axis, rot_angle * scale)
		# else: # denom ~ 0 means input had ~no theta change; do nothing this frame
			# __log_("vert mouse move skip frame")
	
	# DEV
	# var theta_after := acos(clamp(free_boom.normalized().dot(Vector3.UP), -1.0, 1.0))
