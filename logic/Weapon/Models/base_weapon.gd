...
func _ready():
	...
	weapon_area.collision_layer = Collision.Layers.WEAPON_AREA
	weapon_area.collision_mask = Collision.Mask.WEAPON_AREA_MASK
...
func __safe_checks() -> bool:
	if not "current_state" in holder:
		push_error("BaseWeapon: holder does not have current_state")
		return false
	if not holder.current_state:
		push_error("BaseWeapon: holder.current_state is null, cannot get hit data")
		return false
	if not holder.current_state.has_method("pack_hit_data"):
		push_error("BaseWeapon: holder.current_state does not have pack_hit_data method, cannot get hit data")
		return false
	return true
