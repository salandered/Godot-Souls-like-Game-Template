...
var _enter_action_ticks: int


func mark_enter_action() -> void:
	_enter_action_ticks = Time.get_ticks_msec()
	_accumulated_time = 0.0


## ignores pause and time_scale
## NOTE: needs mark_enter_action to be set beforehand
func get_real_time_spent() -> float:
	var now := Time.get_ticks_msec()
	return (now - _enter_action_ticks) / 1000.0 # Convert ms to seconds


## NOTE: needs mark_enter_action to be set beforehand
## respects pause and time_scale
## DANGER: works only if ActionTimeManagement descendant uses accumulate_time_spent() _process
## 		 see accumulate_time_spent
