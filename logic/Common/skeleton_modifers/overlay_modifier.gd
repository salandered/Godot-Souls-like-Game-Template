...
var current_bone_mask: Array[int]
...
func set_overlay_anim(anim: AnimationData, overlay_config: OverlayConfig):
	...
	current_bone_mask = overlay_config.get_bone_mask()
...
func _apply_overlay():
	...
	if current_bone_mask.is_empty():
		__log_("current_bone_mask is empty, using default bone mask")
		bones_to_modify = default_bone_mask
	else:
		bones_to_modify = current_bone_mask
...
func __log_process_start(...parts: Array):
	if overlay_playback.time_spent < custom_delta * 2 and __LOG_PROCESS_START_B:
		...
		__log_("Bone mask:", current_bone_mask)
