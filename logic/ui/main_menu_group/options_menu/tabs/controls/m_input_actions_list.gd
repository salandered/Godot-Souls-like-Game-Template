...
## Manages data storage for new keybindings using a two-level system.
## 1. Immediate (In-Session): Updates Godot's global 'InputMap' so the new control works right away. 
##     This data is lost when the game closes.
## 2. Permanent (Saved): Delegates the final key configuration to a global script (like M_AppSettings) 
##    to be written to a persistent file.
...
@export_group("Custom Names")
@export var custom_key_name_map: Dictionary = {
	"Right Mouse Button": "RMB",
	"Left Mouse Button": "LMB",
	"Middle Mouse Button": "MMB",
	"Mouse Wheel Up": "Wheel Up",
	"Mouse Wheel Down": "Wheel Down",
	"Mouse Thumb Button 1": "MB 4",
	"Mouse Thumb Button 2": "MB 5",
}
...
func _update_assigned_inputs_and_button(action_name: String, action_group: int, input_event: InputEvent) -> void:
	...
	new_readable_input_name = _get_customized_key_name(new_readable_input_name)
...
func _add_action_options(action_name: String, readable_action_name: String, input_events: Array[InputEvent]) -> void:
	...
	for group_iter in range(action_groups):
		...
		text = _get_customized_key_name(text)
		...
		if input_icon_mapper:
			icon = input_icon_mapper.get_icon(input_event)
		# commented because godot didn't like it and i dont use icons here for now
...
func _get_all_action_names(include_built_in: bool = false) -> Array[StringName]:
	...
	if show_all_actions:
		var all_actions := M_AppSettings.get_action_names(include_built_in)
...
func _assign_input_event_to_action_group(input_event: InputEvent, action_name: String, action_group: int) -> void:
	...
	M_AppSettings.set_config_input_events(action_name, final_action_events)
...
func reset() -> void:
	M_AppSettings.reset_to_default_inputs()
	_build_assigned_input_events()
...
func _refresh_action_row(action_name: String) -> void:
	...
	for i in range(action_groups):
		_update_next_button_disabled_state(action_name, i)
