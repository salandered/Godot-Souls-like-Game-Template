...
@abstract
class_name BaseDevVisualizeProcess3D
...
## completely turn off while initializing in game
...
## visualizes only if returns true
@abstract func _conditions_to_visualize() -> bool
...
@abstract func _process_visualization(delta: float) -> void
...
func _process(delta: float) -> void:
	...
	if _conditions_to_visualize():
		_process_visualization(delta)
