...
@abstract
class_name DTCSignalEnabledNode3D
...
@export var _key_b_overlay_panel: DTS.KeyBOverlayPanel = DTS.KeyBOverlayPanel.UNKNOWN
...
func initialize() -> void:
	...
	_initialize_implementation_both_editor_and_game()

	if eu.is_editor():
		return

	## ALL FOLLOWING IS IN GAME
	_initialize_implementation_in_game()
	...
	if react_to_dvc_signal:
		...
		match dvc_section:
			...
			DTS.DTSection.B_OVERLAY_PANEL:
				if _key_b_overlay_panel == DTS.KeyBOverlayPanel.UNKNOWN:
					__log_warn_soft("looks like u forgot to specify _key_b_overlay_panel", "", "", _key_b_overlay_panel)
				sig_pair_to_connect = [GlobalUIInfo.SIG_dtc_b_overlay_panel_value_changed, _on_SIG_dtc_overlay_panel_changed]
			_:
...
func _on_SIG_dtc_overlay_panel_changed(payload: Dictionary[StringName, Variant]):
	var _r := DTCSIGPayloadParser.safe_bget_value_by_dtc_key(payload, _key_b_overlay_panel)
	if _r.err:
		return
