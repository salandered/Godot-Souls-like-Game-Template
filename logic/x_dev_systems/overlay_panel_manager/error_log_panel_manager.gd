...
func get_ui_panel() -> Container:
	return logs_panel.error_logs_panel if logs_panel else null


func get_text_label() -> RichTextLabel:
	return logs_panel.error_logs_label if logs_panel else null


func get_dvc_op_key() -> DVS.KeyBOverlayPanel:
	return DVS.KeyBOverlayPanel.ERROR_LOG
...
func _on___SIG_error_log_printed(payload: Dictionary[StringName, Variant]) -> void:
	...
	var regex_result := _apply_regex_filter(
		...
		DVS.KeySValueChanger.ERROR_LOG_FILTER
	)
