...
func iterate(node: Node) -> void:
	...
	if node is ImporterMeshInstance3D:
		...
		for index in mesh.get_surface_count():
			...
			# my comment: added .contains
			if material_name.ends_with('PixPal') or material_name.contains('PixPal'):
...
func get_pixpal_material() -> ShaderMaterial:
	if not pixpal_material:
		pixpal_material = load("res://addons/pixpal_tools/Imphenzia/PixPal/Materials/IMPixPal.tres")
