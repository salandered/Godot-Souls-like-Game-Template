...
var h_split_container := HBoxContainer.new()
var info_margin_container := MarginContainer.new()
...
var console_height_ratio := 0.2 # 0.3 for 30% height, etc.
...
func _enter_tree() -> void:
	...
	line_edit.anchor_top = console_height_ratio
	line_edit.anchor_right = 1.0
	line_edit.anchor_bottom = console_height_ratio
...
func _setup_layout() -> void:
	...
	panel.anchor_bottom = console_height_ratio
	
	# Add the HBoxContainer to manage horizontal flow
	panel.add_child(h_split_container)
	...
	h_split_container.add_child(rich_label)
	h_split_container.add_child(info_margin_container)
	ControlUtils.margin_container_set_margins(info_margin_container, 10, 20, 20, 10)
	info_margin_container.add_child(info_label)
...
func _setup_info_label() -> void:
	...
	info_label.size_flags_horizontal = Control.SIZE_SHRINK_END
	info_label.text = controls_info


func _update_font_size():
	if font_size > 0:
		line_edit.add_theme_font_size_override("font_size", font_size)
		for label in [rich_label, info_label]:
		...
		for label in [rich_label, info_label]:
			...
			label.remove_theme_font_size_override("bold_italics_font_size")
			label.remove_theme_font_size_override("italics_font_size")
			label.remove_theme_font_size_override("mono_font_size")
...
const controls_info = "[color=light_blue]Up[/color]/[color=light_blue]Down[/color] to navigate commands history
	[color=light_blue]PageUp[/color]/[color=light_blue]PageDown[/color] to scroll registry
	[[color=light_blue]Ctrl[/color] + [color=light_blue]~[/color]] to change console size (half screen; full screen)
	[[color=light_blue]Ctrl[/color] + [color=light_blue]Mouse Wheel[/color]] up/down to change font size
	[color=light_blue]~[/color] or [color=light_blue]Esc[/color] to close
	[color=light_blue]Tab[/color] to autocomplete or cycle matching suggestions
"
