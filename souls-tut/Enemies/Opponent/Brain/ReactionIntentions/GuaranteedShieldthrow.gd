...
func can_throw() -> bool:
	return beliefs.have_stamina_for_state() and beliefs.have_state_charge()


func will_hit() -> bool:
	return beliefs.player_is_missing_attack() or beliefs.player_is_reloading_state() # or can_hit_roll
...
#func will_hit() -> bool:
	...
	#var state_arrival_time = (distance / beliefs.state_speed()) + beliefs.shield_throw_projectile_emit_timing()
	##print("state will be there in " + str(state_arrival_time))
	#var difference = state_arrival_time - snipe_timing
	##if difference < 0.01 and difference > 0:
		##print("will hit state!")
...
#func hit_predicted(time_pos : Array) -> bool:
	...
	#distance -= beliefs.shield_throw_arm_length()
	#var state_arrival_time = distance / beliefs.state_speed()
	#var difference = Time.get_unix_time_from_system() + state_arrival_time - time_pos[0]
