...
func set_animations():
	if full_body_mode:
		...
		#if legs_animator.current_animation != model.current_state.animation + "_legs":
			#set_legs_animation(model.current_state.animation)
			#set_torso_animation(model.current_state.animation)
			#synchronize_if_needed()
	else:
		set_legs_animation(model.legs.current_legs_state.animation)
		set_torso_animation(model.current_state.animation)
		#if legs_animator.current_animation != model.legs.current_legs_state.animation + "_legs":
			#set_legs_animation(model.legs.current_legs_state.animation)
		#if torso_animator.current_animation != model.current_state.animation + "_torso":
			#set_torso_animation(model.current_state.animation)
