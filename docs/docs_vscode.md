...
> [!NOTE]
> I haven't tried [Rider](https://www.jetbrains.com/lp/rider-godot/)
...
- From my experience, [Version Control plugin](https://github.com/godotengine/godot-git-plugin) may break on project reload and has some other issues.
- No code formatting (**godot-tools** has this while with limitations).
- Ability to work with textual representation of files like `.tscn` or `.tres`
- VSCode is just much faster. Even basic actions like code typing or file view scrolling have a noticeble lag in Godot.
- Extensions. Assume you decided to add a markdown linter. In Godot this is probably can't be done, in VSCode this is a usual IDE routine.
...
⚠️ Don't do any write operations to file system via VSCode.
...
Notable example is when you rename a function, _all the mentions of this name in comments_ will be renamed as well. This can be handy, but also deceiving:
...
Let's renaming `func waiting()` to `async_waiting`:
