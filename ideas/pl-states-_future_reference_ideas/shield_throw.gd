...
func spawn_spell() -> void:
	...
	new_shield_shot.caster = _player
	new_shield_shot.add_to_group("players_spell")
	add_child(new_shield_shot)
	new_shield_shot.global_position = _player.global_position # left_wrist.global_position
	new_shield_shot.set_direction(_player.basis.z)
	casted = true
