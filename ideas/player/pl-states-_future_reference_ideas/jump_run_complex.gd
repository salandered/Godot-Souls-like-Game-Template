...
# 	var treshhold = 0.05
# 	if current_action.time_remaining() <= treshhold:
# 		print_.psm_check_trans(state_name, pp.compare_w("time remaining <", "", treshhold) + "=> midair")
...
# region: more control: also extract both Y and XZ components and blend them:
