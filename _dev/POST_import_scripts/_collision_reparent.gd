...
static func reparent_collisions(scene):
	...
	if len(col_collections) == 0:
		__log_pi.info_("ℹ️ FIND_COL_COLLECTION", "found 0 collections ✖️", "Used prefixes for search:", pp.list_(PIConfig.COLLISION_COLLECTION_PREFIXES))
		return
	elif len(col_collections) > 1:
		__log_pi.error_("FIND_COL_COLLECTION", "found multiple collections", "_post_import",
	...
	__log_pi.info_("FIND_COL_COLLECTION", "> Found col collection:", col_collection.name)
	__log_pi.info_("FIND_COL_COLLECTION", "> Its position:", col_collection.position)
	...
	for item in col_collection.get_children():
		if item is StaticBody3D:
			...
			if not has_collision:
				__log_pi.error_("POPULATE_BODIES", "static_body has no CollisionShape3D", "validating bodies", "script hard stop", "body:" + static_body.name)
				return
			var normalized_name = ___normalise_node_name(static_body.name)
			name_to_static_object[normalized_name] = static_body
			__log_pi.info_("POPULATE_BODIES", "> Found static body:", static_body.name, pp.in_sq("normalized:" + normalized_name), "at position:", static_body.position)
	
	__log_pi.info_("POPULATE_BODIES", "> Total static bodies under collision collection:", name_to_static_object.size())
	...
	if meshes_to_reparent.is_empty():
		__log_pi.info_("MATCH_MESHES", "> No matching meshes found to reparent.")
	else:
		__log_pi.info_("MATCH_MESHES", "\n  > Found", meshes_to_reparent.size(), "matches. Starting reparent...")
	...
	for static_body_name: String in name_to_static_object:
		if not meshes_with_static_match_found.has(static_body_name):
			__log_pi.error_("MATCH_CHECK", "no mesh found for static body", "collision matching", "continuing without match", "static body name:", pp.in_q(static_body_name))
	
	# CLEANUP: Remove empty col collection
	if col_collection.get_child_count() == 0:
		__log_pi.info_("CLEANUP_COL_COLLECTION", "> Removing col collection:", col_collection.name)
...
static func perform_combined_reparent(visual_mesh: MeshInstance3D, static_body: StaticBody3D, col_collection: Node) -> void:
	var __prefix = "👪 3"
	__log_pi.info_(__prefix, "\n=== REPARENTING:", visual_mesh.name, "===")
	
	var body_local_pos = static_body.position
	var mesh_local_pos = visual_mesh.position
	var mesh_parent = visual_mesh.get_parent()
	
	__log_pi.info_(__prefix, "[BEFORE] Static body local pos:", body_local_pos)
	__log_pi.info_(__prefix, "[BEFORE] Visual mesh local pos:", mesh_local_pos)
	__log_pi.info_(__prefix, "[BEFORE] Mesh parent:", mesh_parent.name)
	__log_pi.info_(__prefix, "[BEFORE] Body name:", pp.in_q(static_body.name))
	__log_pi.info_(__prefix, "[BEFORE] Mesh name:", pp.in_q(visual_mesh.name))
	
	# Calculate the offset between visual mesh and static body
	var offset = mesh_local_pos - body_local_pos
	__log_pi.info_(__prefix, "[OFFSET] Calculated offset:", offset)
	
	if visual_mesh.mesh == null:
		__log_pi.error_(__prefix, "mesh instance has no mesh resource", "perform_combined_reparent", "script hard stop", "mesh name:", visual_mesh.name)
		return
	
	var target_owner = static_body.owner
	if target_owner == null:
		__log_pi.error_(__prefix, "static body has no owner", "perform_combined_reparent", "script hard stop", "body:", static_body.name)
		return
	...
	__log_pi.info_(__prefix, "[STEP 1] Renamed mesh:", original_mesh_name, "->", visual_mesh.name)
	...
	__log_pi.info_(__prefix, "[STEP 2] Moved body to:", mesh_parent.name)
	...
	__log_pi.info_(__prefix, "[STEP 3] Moved mesh under body")
	
	__log_pi.info_(__prefix, "[AFTER] Visual mesh local pos:", visual_mesh.position)
	__log_pi.info_(__prefix, "[AFTER] Static body local pos:", static_body.position)
