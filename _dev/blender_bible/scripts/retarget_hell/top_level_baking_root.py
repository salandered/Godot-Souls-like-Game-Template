...
import root_bone_copies_empty_location
...
def reload_helpers():
    ...
    importlib.reload(root_bone_copies_empty_location)
...
def find_retarget_name(base_name: str, prefix_to_remove: str):
    ...
    # Remove the specified prefix from the base_name if it exists.
    ...
    # Construct the single, case-sensitive candidate name.
    # The postfix must be exactly "Retarget".
    candidate_name = f"{cleaned_base_name} Retarget"
    
    # Check if an action with this exact name exists and return it.
...
def process_action(action_name: str, retarget_name: str):
    ...
    # copy root bone location from Empty
    ...
    # bake root bone over scene range (visual, clears constraint)
